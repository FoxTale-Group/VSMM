...
ModEntry::ModEntry(const QJsonObject &json, QString version, QString modId, QString filename)
    : mVersion{std::move(version)}, mModId{std::move(modId)}, mFilename{std::move(filename)}, mHasInfoReceived{true} {
    initName(json);
    initModUrl(json);
    initAuthor(json);
    initTags(json);
...
ModEntry::ModEntry(QString name, QString version, QString author, QString modId, QString filename)
    : mName(std::move(name)), mVersion(std::move(version)), mAuthor(std::move(author)), mModId(std::move(modId)),
      mFilename(std::move(filename)) {}

QAnyStringView ModEntry::getId() const { return mModId; }

QAnyStringView ModEntry::getName() const { return mName; }

QAnyStringView ModEntry::getAuthor() const { return mAuthor; }

QAnyStringView ModEntry::getVersion() const { return mVersion; }

const QUrl &ModEntry::getUrl() const { return mUrl; }

QAnyStringView ModEntry::getUpdateVersion() const { return mUpdateVersion; }

const QStringList &ModEntry::getTags() const { return mTags; }

const QUrl &ModEntry::getLatestVersionUrl() const { return mLatestVersionUrl; }

QAnyStringView ModEntry::getType() const { return mType; }

bool ModEntry::hasUpdate() const { return mHasUpdate; }

bool ModEntry::hasInfoReceived() const { return mHasInfoReceived; }

void ModEntry::initName(const QJsonObject &json) { mName = json["name"].toString().trimmed(); }
...
void ModEntry::initUpdateVersion(const QJsonObject &json) {
    ...
    try {
        semver::version latestReleaseVersion =
...
void ModEntry::initAuthor(const QJsonObject &json) { mAuthor = json["author"].toString(); }
...
void ModEntry::initType(const QJsonObject &json) { mType = json["type"].toString(); }
} // namespace vsmodchecker
